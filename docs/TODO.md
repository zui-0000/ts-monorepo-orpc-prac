...
### デザインシステムと UI コンポーネントの方針を決める

**状態**: 検討中 (2026-09-07 起票)

#### 動機

**CSS を 1 行も書いていない。** features の実装に入る前に、UI の土台と判断基準を
決めておきたい。トークンの粒度もコンポーネントの境界も**測れば決まる類ではない**ため、
先に言葉にしておかないと実装しながら毎回揺れる。

#### 決めること

- **デザインシステムそのもの** — デジタル庁が公開しているデザインシステムを参考に
  組み立てるのが候補
- **UI と UX の方針・原則** — 画面を作るときの判断基準。「迷ったらこう寄せる」を
  言葉にしておく
- **トークンの範囲** — 色や z-index をどこまでデザインシステムに組み込むか。
  余白や字送りも含めるか
- **UI コンポーネントの供給元** — shadcn/ui か Base UI。`packages/ui` へ切り出す

#### 分かったこと

**shadcn/ui と Base UI は同じ層のものではない。「どちらか」で選ぶ対象ではない。**

|           | 形                                               | 層                     |
| --------- | ------------------------------------------------ | ---------------------- |
| shadcn/ui | コピーして自分のリポジトリに置くコンポーネント集 | 見た目まで含む         |
| Base UI   | npm で入れるヘッドレスの部品                     | 挙動とアクセシビリティ |

shadcn は土台に Radix か Base UI を選べる。**2026 年 7 月以降の新規プロジェクトは
Base UI が既定**になっており、Radix も引き続き使える。したがって「shadcn を入れて
土台に Base UI を敷く」も成立する。採用前にここを確かめること。

#### 想定する形

```txt
packages/ui              UI コンポーネント。見た目と挙動だけを持つ
apps/frontend/src/components/
├── features/            機能単位。packages/ui を import して組む
└── pages/               features を並べて画面にする
```

`pages/` を `routes/` へ下ろさない方針はこの形を前提にしている
(docs/04-ルーティングの仕組み.md)。

#### 順番がある

**デザインシステム → `packages/ui` → features → モック画面**、の順でしか進めない。
前が決まらないと後ろの粒度が決まらないため。
...
- **見た目** — CSS を 1 行も書いていない。素の HTML のまま。方針は上の検討中で扱う。
  `PageLoadingSpinner` は回らないので、スタイルを入れるか改名するかを決める
- **features の機能コンポーネント** — まだ 1 つも無い。`packages/ui` が決まってから
- **MSW のモック用画面** — ハンドラは揃っているが、それを叩く画面が
  サインイン・サインアップ・プロフィールの 3 枚しかない
