...
`oxc-transform-react` は最新が `0.149.0` だが `^0.145.0` で止めてある。
`@vitejs/plugin-react@6.1.1` の peer がそれを要求しており、**0.x では `^` が
マイナー固定**のため `0.149` は満たせない。plugin 側が上げたら追う。

**上げても得るものが無い。** `0.148.0` で試したところ、バンドルは
`925.80 kB` でハッシュまで一致し、最適化された数も 12 のまま変わらなかった。
peer を外してまで追う理由が無いため、`pnpm check:updates` の報告は無視する。
