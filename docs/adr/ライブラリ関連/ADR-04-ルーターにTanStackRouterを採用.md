...
TanStack Router には経路の組み方が 2 つある。**ファイルベースを採り、コードベースは
採らない。**

- **ファイルベース** — `routes/` のファイル構成から `routeTree.gen.ts` を生成する
- **コードベース** — `createRoute({ getParentRoute })` で木を手で組む

公式が「**ファイルベースが推奨される方法**」と明言しており、利点として簡潔さ、URL を
写す構成、規模への追随、コード分割、型安全、一貫性の 6 つを挙げている。コードベースを
選ぶべき場面は挙げられていない。

このリポジトリで効いているのは次の 3 つで、いずれも**コードベースでは得られない**。

| 得ているもの      | 実際の効果                                                            |
| ----------------- | --------------------------------------------------------------------- |
| 型生成            | `routeTree.gen.ts` が経路の union を作る。`to` が既知パスに限定される |
| 自動コード分割    | 走査対象が `routes/` のファイルのため。画面が経路ごとに分かれる       |
| 構成が URL を写す | ファイルを見れば経路が分かる。木の組み立てを読む必要が無い            |

**代償は生成物を 1 つ抱えることと、`__root.tsx` という固定の名前を受け入れること。**
どちらも許容した。生成物は git に入れて lint と整形から外してあり、扱いは
[docs/04](../../04-ルーティングの仕組み.md) に書いた。

コードベースが要るのは、経路を実行時に組み立てたい場合である。現状そうした要件は無い。
