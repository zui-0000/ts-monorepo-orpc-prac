...
> **この `uidx` の判断は後に覆っている (ADR-03 / 2026-08-29)。** better-auth が索引名を
> 自動生成するのは CLI 生成器と内蔵マイグレータの経路だけで、スキーマを手書きする
> 本構成では発火しないことが判明したため。現在の規約は Postgres の種別接尾辞
> (`_idx` / `_key`) をそのまま使う。上記は**当時の判断の記録**として残している。
